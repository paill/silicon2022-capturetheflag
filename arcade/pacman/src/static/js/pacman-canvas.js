		...
		this.getNextDirection = function () {
			...
			if (this.dead) {
				for (var i = dirs2.length - 1; i >= 0; i--) {
					if ((dirs2[i].field != "wall") && (dirs2[i].field != "xwall") && !(dirs2[i].dir.equals(this.getOppositeDirection()))) {
						r = dirs2[i].dir;
					}
				}
			} else {
				for (var i = dirs2.length - 1; i >= 0; i--) {
					if ((dirs2[i].field != "wall") && (dirs2[i].field != "xwall") && (dirs2[i].field != "door") && !(dirs2[i].dir.equals(this.getOppositeDirection()))) {
						r = dirs2[i].dir;
					}
				}
			}
			...
			if ((this.stuckX == 0) && (this.stuckY == 0) && this.frozen == false) {
				...
				if ((field === "pill") || (field === "powerpill")) {
					...
					if (
						...
						(fieldAhead === "wall" || fieldAhead === "xwall")
					) {
						var s;
						if (field === "powerpill") {
				...
				if ((fieldAhead === "wall") || (fieldAhead === "xwall") || (fieldAhead === "door")) {
			...
			if (this.directionWatcher.get() != null) {
				...
				if ((this.stuckX == 1) && this.directionWatcher.get() == right) this.directionWatcher.set(null);
					...
					if ((this.inGrid())) {
						...
						if (nextTile != "wall" && nextTile != "xwall") {
							this.setDirection(this.directionWatcher.get());
							this.directionWatcher.set(null);
						}
					}
		...
		if (game.map && game.map.posY && game.map.posY.length > 0) {
			$.each(game.map.posY, (i, row) => {
				...
				$.each(row.posX, (j, column) => {
					...
					else if (column.type == "xwall") {
						context.fillStyle = "Black";
						context.strokeStyle = "Black";
						context.fillRect(pacman.radius / 2 + (column.col - 1) * 2 * pacman.radius, pacman.radius / 2 + (dotPosY - 1) * 2 * pacman.radius, pacman.radius, pacman.radius);
						context.fillStyle = "White";
						context.strokeStyle = "White";
					}
				});
